...
- Canvas-вариант, геометрия «как у источника» (иначе клики уезжают мимо):
  ...
  * строки стекаются по `widget.last_y` источника, когда runtime их даёт
    (фолбэк: заявленные высоты + PORTAL_ROW_GAP=4);
...
app.js/LiteGraph, копия реальных `web/*.js` в песочницу. Фазы A–T покрывают
...
геометрию/клики порталов (масштаб-компенсация, last_y-стек, клип-рост).

```bash
node scripts/smoke_hub.mjs   # базовая линия: >=233 зелёных, 0 упавших
```

...
- Canvas-вариант: буфер держится 1:1 с CSS-размером (никакого растягивания
  стилями!), ширина капится натуральной шириной источника, тикер ~12fps +
  `runPortalTicks(node)` в тестах. Групповые embeds: один canvas на всю
  панель, members[] в item, мышь маршрутизируется по y-диапазонам членов.
...
app.js/LiteGraph, копия реальных `web/*.js` в песочницу. Фазы A–R покрывают
детекцию типов, зеркала, write-through, пресеты, табы, DnD, multiline,
layout, порталы, whole-panel группы, пиннинг, searchable combo.

```bash
node scripts/smoke_hub.mjs   # базовая линия: >=197 зелёных, 0 упавших
```
